...
public class CitaServlet extends HttpServlet {
    ...
    private void doGetRequestIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Cita cita = new Cita(); // Crear una instancia  de la entidad de Servicio.
            cita.setTop_aux(10); // Agregar el Top_aux con el valor de 10 a la propiedad Top_aux de Servicio.
            ArrayList<Cita> citas = CitaDAL.buscar(cita); // Ir a la capa de acceso a datos y buscar los registros de Rol.
            // El request.setAttribute se utiliza para enviar datos desde un servlet a un jsp.
            request.setAttribute("citas", citas); // Enviar los roles al jsp utilizando el request.setAttribute con el nombre del atributo servicios.
            ...
            request.getRequestDispatcher("Views/Cita/index.jsp").forward(request, response); // Direccionar al jsp index de Servicio.
        } catch (Exception ex) {
    ...
    private void doPostRequestIndex(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Cita cita = obtenerCita(request); // Llenar la instancia de Servicio con los parámetros enviados en el request 
            ArrayList<Cita> citas = CitaDAL.buscar(cita); // Buscar los servicios que cumple con los datos enviados en el request
            request.setAttribute("citas", citas); // Enviar los servicios al jsp utilizando el request.setAttribute con el nombre del atributo roles
            // Enviar el Top_aux de Servicio al jsp utilizando el request.setAttribute con el nombre del atributo top_aux
            request.setAttribute("top_aux", cita.getTop_aux());
            request.getRequestDispatcher("Views/Cita/index.jsp").forward(request, response); // Direccionar al jsp index de Servicio
        } catch (Exception ex) {
    ...
    private void requestObtenerPorId(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            if (cita_result.getId() > 0) { // Si el Id es mayor a cero.
                ...
                Utilidad.enviarError("El Id:" + cita.getId() + " no existe en la tabla de Servicio", request, response);
            }
        } catch (Exception ex) {
            // enviar al jsp de error si hay un Exception
            Utilidad.enviarError(ex.getMessage(), request, response);
